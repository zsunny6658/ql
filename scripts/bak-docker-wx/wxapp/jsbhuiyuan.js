...
async function getWxCode(appid, openid) {
    const { status, data } = await request({
        ...
        data: { wxid: "zx491814", appid },
    });
    const code = data?.Data?.code || data?.code || data?.data?.code || data?.phoneCode || data?.data?.phoneCode;
...
class JsbHuiyuan {
    ...
    async login() {
        ...
        const ct = data.clientToken || data.data?.clientToken || data.data?.data?.clientToken;
        if (ct) this.global.clientToken = ct;
