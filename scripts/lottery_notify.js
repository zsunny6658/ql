                ...
                if (isGzip) {
                    zlib.gunzip(buffer, (err, decoded) => {
                        if (err) {
                            ...
                            // ==================== 新增通知调用 ====================
                            sendNotify('龙湖天街抽奖失败', `解压响应数据时出错: ${buffer.toString()}`);
                            // ==================== 新增结束 ====================
                            ...
                            try {
                                ...
                                if (jsonData.code === '0000') {
                                    ...
                                    if (jsonData.data) {
                                        ...
                                        sendNotify('龙湖天街抽奖成功', `今日抽奖成功！获得奖品类型: ${jsonData.data.item_no || '无'};奖励类型: ${jsonData.data.reward_type || '未知'};奖励数量: ${jsonData.data.reward_num || 0}`);
                                    }
                                } else {
                                    console.log(`抽奖失败: ${jsonData.message || '未知错误'}`);
                                    sendNotify('龙湖天街抽奖失败', `${jsonData.message || '未知错误'}`);
                                }
                                
                                resolve(jsonData);
                            } catch (e) {
                                console.log(`非JSON响应数据:`, decoded.toString());
                                sendNotify('龙湖天街抽奖异常', `响应数据解析异常: ${decoded.toString()}`);
                    ...
                    try {
                        ...
                        sendNotify('龙湖天街抽奖异常', `响应数据解析异常: ${buffer.toString()}`);
                        resolve(buffer.toString());
                    }
                }
            });
