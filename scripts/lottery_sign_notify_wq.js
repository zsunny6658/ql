                ...
                if (isGzip) {
                    zlib.gunzip(buffer, (err, decoded) => {
                        if (err) {
                            ...
                            // ==================== 新增通知调用 ====================
                            sendNotify('WQ龙湖天街抽奖签到失败', `解压响应数据时出错: ${buffer.toString()}`);
                            // ==================== 新增结束 ====================
                            ...
                            try {
                                ...
                                if (jsonData.code === '0000') {
                                    ...
                                    sendNotify('WQ龙湖天街签到成功', `今日抽奖签到成功！获得抽奖机会: ${jsonData.data ? jsonData.data.chance : '未知'}次`);
                                } else {
                                    console.log(`抽奖签到失败: ${jsonData.message || '未知错误'}`);
                                    sendNotify('WQ龙湖天街抽奖签到失败', `${jsonData.message || '未知错误'}`);
                                }
                                
                                resolve(jsonData);
                            } catch (e) {
                                console.log(`非JSON响应数据:`, decoded.toString());
                                sendNotify('WQ龙湖天街抽奖签到异常', `响应数据解析异常: ${decoded.toString()}`);
                    ...
                    try {
                        ...
                        if (jsonData.code === '0000') {
                            console.log('🎉 抽奖签到成功！');
                            console.log(`获得抽奖机会: ${jsonData.data ? jsonData.data.chance : '未知'}次`);
                            sendNotify('WQ龙湖天街抽奖签到成功', `今日签到成功！获得抽奖机会: ${jsonData.data ? jsonData.data.chance : '未知'}次`);
                        } else {
                            console.log(`抽奖签到失败: ${jsonData.message || '未知错误'}`);
                            sendNotify('WQ龙湖天街抽奖签到失败', `${jsonData.message || '未知错误'}`);
                        }
                        ...
                        sendNotify('WQ龙湖天街抽奖签到异常', `响应数据解析异常: ${buffer.toString()}`);
                        resolve(buffer.toString());
                    }
                }
            });
