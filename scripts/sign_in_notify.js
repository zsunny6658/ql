...
async function signIn() {
    const token = process.env.LONG_FOR_TOKEN || '8132c0dac2e94fa0b8a52e91def36d3e';
    const risktoken = process.env.LONG_FOR_RISK_TOKEN || '6a6d5931l7EfNuNBVItoD0s8dAIBdI1lpikHAyR1';
    ...
    const headers = {
        ...
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_6_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.75(0x18004b4f) NetType/WIFI Language/zh_CN miniProgram/wx50282644351869da',
                ...
                if (isGzip) {
                    zlib.gunzip(buffer, (err, decoded) => {
                        if (err) {
                            ...
                            // ==================== 新增通知调用 ====================
                            sendNotify('龙湖天街签到失败', `解压响应数据时出错: ${buffer.toString()}`);
                            // ==================== 新增结束 ====================
                            ...
                            try {
                                ...
                                if (jsonData.code === '0000') {
                                    if (jsonData.data && jsonData.data.is_popup === 1) {
                                        ...
                                        sendNotify('龙湖天街签到成功', `今日签到成功！获得成长值: ${jsonData.data.reward_info ? jsonData.data.reward_info[0].reward_num : '未知'}`);
                                    } else {
                                        console.log('✅ 今日已签到');
                                        // ==================== 新增通知调用 ====================
                                        sendNotify('龙湖天街签到', '今日已签到，无需重复签到');
                                        // ==================== 新增结束 ====================
                                    }
                                } else {
                                    console.log(`签到失败: ${jsonData.message || '未知错误'}`);
                                    sendNotify('龙湖天街签到失败', `${jsonData.message || '未知错误'}`);
                                }
                                
                                resolve(jsonData);
                            } catch (e) {
                                console.log(`非JSON响应数据:`, decoded.toString());
                                sendNotify('龙湖天街签到异常', `响应数据解析异常: ${decoded.toString()}`);
