...
class Task {
    constructor(env) {
        ...
        // 支持 JSON 格式传 token + refreshToken：{"token":"***","refreshToken":"***"}
        if (!this.isLegacyToken) {
            try {
                const parsed = JSON.parse(this.raw);
                if (parsed && parsed.token && parsed.refreshToken) {
                    this.token = parsed.token;
                    this.refreshToken = parsed.refreshToken;
                    this.accountId = `json:${shortToken(this.token)}`;
                    $.log(`账号[${this.index}] JSON token 已注入`);
                }
            } catch (e) {
                // 不是 JSON，继续走正常登录流程
            }
    ...
    async run() {
        if (this.isLegacyToken) {
            $.log(`账号[${this.index}] 检测到 token 格式: ${shortToken(this.token)}`);
            this.saveCachedToken();
            if (!(await this.checkToken())) {
                this.removeCachedToken();
                $.log(`账号[${this.index}] token失效`);
                return;
            }
        } else {
            const cached = this.getCachedToken();
            if (this.token) {
                // 已有 token（JSON 注入 或 缓存），直接缓存并继续
                if (!this.accountId.startsWith('json:')) {
                    this.applyToken(cached);
                }
                this.saveCachedToken();
                $.log(`账号[${this.index}] 使用token: ${shortToken(this.token)}`);
                if (!(await this.checkToken())) {
                    this.removeCachedToken();
                    $.log(`账号[${this.index}] token失效`);
                }
    ...
    async request(apiPath, { method = "GET", data, params, auth = true } = {}) {
        ...
        if (headers?.["x-access-token"]) {
            this.token = this.token || headers["x-access-token"];
            this.refreshToken = headers["x-access-token"];
        }
    ...
    async getLoginCode() {
        const { data } = await wechat.getCode("");
    ...
    async loginByWxCode() {
        try {
            ...
            const bodyRefreshToken = result?.Data?.RefreshToken || result?.Data?.refreshToken || "";
            if (bodyToken) {
                this.token = bodyToken;
                this.refreshToken = bodyRefreshToken;
                this.saveCachedToken();
                $.log(`账号[${this.index}] CODE登录成功(body): ${shortToken(this.token)}`);
                return;
            }
            if (this.token) {
                this.saveCachedToken();
                $.log(`账号[${this.index}] CODE登录成功(header x-access-token): ${shortToken(this.token)}`);
                return;
            }
            throw new Error(`OnLogin未返回业务token (OpenId=${this.openId || "空"}，x-access-token=${this.refreshToken || "空"})`);
        } catch (e) {
