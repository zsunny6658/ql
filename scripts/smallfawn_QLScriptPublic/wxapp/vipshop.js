...
async function getWxCode(openid) {
  const { status, data } = await request({
    ...
    data: { wxid: "zx491814", appid: MINI_APP_ID },
  });
  const s = { status, data: data || {} };
  const code = s.data?.Data?.code || s.data?.code || s.data?.data?.code;
  if (s.status !== 200 || !code) throw new Error(`获取 code 失败 HTTP ${s.status}: ${short(s.data)}`);
  return code;
}
