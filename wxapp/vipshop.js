...
const DEFAULT_MARS_CID = process.env.vipshop_mars_cid || "1785481274112_e80df2aa94e119919a8ea458b4e7803e";
const DEFAULT_WAREHOUSE = "VIP_SH";
const DEFAULT_AREA = "103103101";
const DEFAULT_TFS_FP = "BjXscSggDVWtkYhmwtKTmvF2aD8kQ68icSe1y8JoDTlGbpMsWaNXjWYBX72ygn2O4Mis0x88/q4iPFmeBvk/ruA==";
const DEFAULT_PROVINCE = "103103";
...
async function request(options) {
  const res = await axios.request({
    ...
    headers: {
      ...
      "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 18_6_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.75(0x18004b4f) NetType/WIFI Language/zh_CN",
...
async function getWxCode(openid) {
  ...
  throw new Error('wx_server_url 未配置，请先设置 YYB 的 wx_server_url (如 http://172.17.0.1:8787)');
}
...
class Vipshop {
  ...
  baseData() {
    return {
      ...
      width: 393,
      height: 852,
      phone_model: "iPhone 15 pro<iPhone16,1>",
      phone_brand: "iPhone",
      sys_version: "iOS 18.6.2",
      is_default_area: "1",
      app_theme_mode: "0",
      app_theme_action: "0",
      req_scene: 0,
  ...
  paramHash(data = {}, method = "POST") {
    ...
    const params = body.split("&").reduce((obj, pair) => {
  ...
  async signInfo() {
    ...
    const _D = data;
    $.log(`DIAG1 vipOpenid=${this.vipOpenid || "<empty>"}`);
    $.log(`DIAG1 token=${this.token ? this.token.slice(0,8)+"***" : "<empty>"}`);
    $.log(`DIAG1 marsCid=${this.marsCid}`);
    $.log(`DIAG1 tfs_fp=${_D.tfs_fp_token ? _D.tfs_fp_token.slice(0,12)+"..." : "<empty>"}`);
    $.log(`DIAG1 t=${_D.t} actId=${_D.actId}`);
    const _D_ph = this.paramHash(data, "POST");
    const _D_sign = this.signHeader(url, data, "POST");
    $.log(`DIAG1 paramHash=${_D_ph}`);
    $.log(`DIAG1 Authorization=${_D_sign.slice(0,90)}`);
    const DIAG_ph = this.paramHash(data, "POST");
    const DIAG_sign = this.signHeader(url, data, "POST");
    const DIAG_t = this.token;
    const DIAG_token_head = DIAG_t ? DIAG_t.slice(0, 10) + "***" : "<empty>";
    $.log(`DIAG vipOpenid=${this.vipOpenid || "<empty>"}`);
    $.log(`DIAG marsCid=${this.marsCid}`);
    $.log(`DIAG token_head=${DIAG_token_head}`);
    $.log(`DIAG tfs_fp=${data.tfs_fp_token ? data.tfs_fp_token.slice(0, 16) + "..." : "<empty>"}`);
    $.log(`DIAG paramHash=${DIAG_ph}`);
    $.log(`DIAG apiSign=${DIAG_sign.slice(0, 80)}`);
    const { status, data: res } = await request({
      method: "POST",
